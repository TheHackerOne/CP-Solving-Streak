...
void mergeSort(vector<ll> &a, ll l, ll h, vector<int> &parent) {
    ll mid = (l + h) / 2;
    ll ele = h - l + 1;
    ll temp[ele];
    ll i = l, j = mid + 1, k = 0;
...
void merge(vector<ll> &a, ll  l, ll  h, vector<int> &parent) {
    if (l >= h) return;
    ll mid = (l + h) / 2;
    merge(a, l, mid, parent);
    merge(a, mid + 1, h, parent);
    mergeSort(a, l, h, parent);
}

ll inversionCount(vector<ll> &arr, ll N, vector<int> &parent)
{
    counter = 0;
    merge(arr, 0, N - 1, parent);
    return counter;
}

ll parentNode(int node, vector<int> &parent){
...
void solve() {
    ...
    int inversion = inversionCount(arr, n, parent);
