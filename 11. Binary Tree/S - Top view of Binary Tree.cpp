...
int topNodesEachLevel[1001];

void topViewOfBT(BinaryTreeNode *root, int hl) {
    ...
    while (!q.empty()) {
        ...
        while (size--) {
            ...
            if (topNodesEachLevel[hl] == INT_MAX)
                topNodesEachLevel[hl] = curr.first -> data;
...
int main() {
    ...
    for (int i = 0; i < width; i++)
        topNodesEachLevel[i] = INT_MAX;
    topViewOfBT(root, abs(mm[0]));
    for (int i = 0; i < width; i++)
        cout << topNodesEachLevel[i] << " ";
