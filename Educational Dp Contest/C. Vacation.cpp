...
void solve() {
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 0; j < 3; j++) {
			if (j == 0) dp[j][i] = max(dp[j + 1][i - 1], dp[j + 2][i - 1]) + a[i - 1][0];
			else if (j == 1) dp[j][i] = max(dp[j - 1][i - 1], dp[j + 1][i - 1]) + a[i - 1][1];
			else dp[j][i] = max(dp[j - 1][i - 1], dp[j - 2][i - 1]) + a[i - 1][2];
		}
	}
	cout << max({dp[0][n], dp[1][n], dp[2][n]}) << nline;
}
