...
int maxOverlaps(vector<int> &start, vector<int> &end){
    int maxIntervals = 1;
    int platforms = 1;
    int i = 1, j = 0;
    ...
    while(i < start.size() and j < end.size()){
        if(start[i] > end[j]){
            ...
            platforms++;
            maxIntervals = max(maxIntervals, platforms);
            i++;
        }
    }
...
void solve() {
    ...
    for(int i=0;i<n;i++){
        int s, e, speed;
        cin>>s>>e>>speed;
        arr.push_back({ { s, e }, speed });
        m1[speed].push_back({ s, e });
    }
