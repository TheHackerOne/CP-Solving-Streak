...
bool isBipartite(int node, unordered_map<int, vector<int>> &mp, vector<bool> &vis, int color, vector<int> &colors){
    vis[node] = true;
    colors[node] = color;
    for(auto neigh:mp[node]){
        if(!vis[neigh]){
            bool res = isBipartite(neigh, mp, vis, !color, colors);
            if(!res) return false;
...
void solve() {
    ...
    for(int i=1;i<=n;i++){
        if(!visited[i]){
            bool res = isBipartite(i, mp, visited, 0, colors);
            if(!res){
                cout<<"NO"<<nline;
                return;
            }
