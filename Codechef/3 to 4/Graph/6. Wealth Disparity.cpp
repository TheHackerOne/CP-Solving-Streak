...
int dfs(int root, unordered_map<int, vector<int>> &graph, vector<bool> &vis, int &maxDiff, vector<int> &wealth){
    vis[root] = true;
    ...
    for(auto neigh:graph[root]){
        if(!vis[neigh]){
            flag = true;
            int res = dfs(neigh, graph, vis, maxDiff, wealth);
            minEle = min(minEle, res);
        }
    }

    if(flag)
        maxDiff = max(maxDiff, wealth[root]-minEle);

    return min(minEle, wealth[root]);
}
...
void solve() {
    ...
    int minEle = dfs(root, graph, visited, maxDiff, wealth);
    cout<<maxDiff<<nline;
}
