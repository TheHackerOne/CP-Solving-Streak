https://www.codechef.com/LP3TO401/problems/RISK
...
void dfs(int i, int j, vector<string> &arr, int &size){
  size++;
  arr[i][j] = '0';
  for(int u=0;u<4;u++){
    int newr = i+r[u], newc = j+c[u];
    if(isValid(newr, newc, arr.size(), arr[0].length()) and arr[newr][newc] == '1'){
      dfs(newr, newc, arr, size);
    }
  }
}
  ...
  for(int i=0;i<n;i++){
    for(int j=0;j<m;j++){
      if(arr[i][j] == '1'){
        int size = 0;
        dfs(i, j, arr, size);
        islands.push_back(size);
      } 
    }
  }

  sort(all(islands), [](int a, int b){
    return a > b;
  });
