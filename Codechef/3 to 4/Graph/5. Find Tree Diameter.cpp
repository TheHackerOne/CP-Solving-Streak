...
int dfs(int node, unordered_map<int, vector<int>> &graph, vector<bool> &vis, int &maxDia){
    vis[node] = true;
    ...
    for(auto neigh:graph[node]){
        if(!vis[neigh]){
            int res = dfs(neigh, graph, vis, maxDia);
            if(res > maxLen){
                smaxLen = maxLen;
                maxLen = res;
            }else if(res > smaxLen){
    ...
    maxDia = max(maxDia, maxLen+smaxLen+1);
    return maxLen+1;
}
...
void solve() {
    ...
    int res = dfs(0, graph, visited, maxDia);
    maxDia = max(maxDia, res);
