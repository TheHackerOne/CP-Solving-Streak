https://www.codechef.com/LP3TO401/problems/ADAKNG
...
vector<int> ro = {-1, -1, 0, 1, 1, 1, 0, -1};
vector<int> co = {0, 1, 1, 1, 0, -1, -1, -1};

bool isValid(int r, int c){
  if(r >= 0 and r < 8 and c >= 0 and c < 8) return true;
  return false;
}
...
void solve() {
  ...
  while(!q.empty()){
    ...
    while(size--){
      ...
      ans++;
      for(int i=0;i<8;i++){
        int newr = cord.first+ro[i], newc = cord.second+co[i];
        if(isValid(newr, newc) and arr[newr][newc] == 0){
          arr[newr][newc] = 1;
          q.push({ newr, newc });
        }
      }
