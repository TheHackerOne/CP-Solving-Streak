...
bool isValid(int newi, int newj, int n, int m){
    if(newi >= 0 and newi < n and newj >= 0 and newj < m) return true;
    return false;
}

void dfs(int i, int j, set<pair<int, int>> &arr, int &res, int r, int c, map<pair<int, int>, bool> &vis){
    vis[{ i, j }] = 1;

    int neigh = 0;
    for(int k=0;k<4;k++){
        int newi = i+rr[k], newj = j+cc[k];
        if(isValid(newi, newj, r, c) and arr.find({ newi, newj}) != arr.end() ){
            neigh++;
        }
    }
    res += (4-neigh);
    ...
    for(int k=0;k<4;k++){
        int newi = i+rr[k], newj = j+cc[k];
        if(isValid(newi, newj, r, c) and !vis[{newi, newj}] and arr.find({ newi, newj}) != arr.end()){
            dfs(newi, newj, arr, res, r, c, vis);
        }
    }
}
...
void solve() {
    ...
    for(auto i:arr){
        if(!visited[{ i.first, i.second }]){
            int res = 0;
            dfs(i.first, i.second, arr, res, r, c, visited);
            debug(res)
            ans = max(ans, res);
        }
    }
