...
ll maxProfit(ll node, vector<int> &points, unordered_map<int, vector<int>> &mp, vector<bool> &vis, ll x){
    vis[node] = true;
    ll ans = 0;
    for(auto neigh:mp[node]){
        if(!vis[neigh])
            ans += maxProfit(neigh, points, mp, vis, x);
    }
    debug(-x)
    debug(ans+points[node])
    return max(-x, ans + points[node]); // delete, not delete
}
...
void solve() {
    ...
    cout<<maxProfit(0, points, mp, visited, x)<<nline;
}
