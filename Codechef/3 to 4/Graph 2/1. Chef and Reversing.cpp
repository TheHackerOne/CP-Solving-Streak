...
int minEdgesReverse(int src, int dest, unordered_map<int, vector<pair<int, int>>> &mp){
    priority_queue<pair<int, int>, vector<pair<int, int>>,  greater<pair<int, int>>> pq;
    pq.push({ 0, src });
    vector<bool> vis(dest+1, false);
    vector<int> distArr(dest+1, INT_MAX);
    ...
    while(!pq.empty()){
        ...
        for(auto neigh:mp[node]){
            if(!vis[neigh.first] and (distance + neigh.second < distArr[neigh.first])){
                distArr[neigh.first] = distance+neigh.second;
                pq.push({ distArr[neigh.first], neigh.first });
            }
        }
    }
    return -1;
...
void solve() {
    ...
    for(int i=0;i<m;i++){
        cin>>u>>v;
        mp[u-1].push_back({v-1, 0});
        mp[v-1].push_back({u-1, 1});
    }

    cout<<minEdgesReverse(0, n-1, mp)<<nline;
}
