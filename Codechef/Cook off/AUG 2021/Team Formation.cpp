...
void solve() {
	...
	string s1, s2;
	cin >> s1 >> s2;
	int oneProLan = 0, oneProEng = 0, twoPro = 0, zeroPro = 0;
	for (int i = 0; i < n; i++) {
		if (s1[i] == '1' && s2[i] == '1')
			twoPro++;
	...
	int totalTeams = 0;
	int mini = min(twoPro, zeroPro);
	totalTeams += mini;
	twoPro -= mini;
	zeroPro -= mini;
	if (twoPro == 0 && zeroPro != 0) {
		int mini = min(oneProEng, oneProLan);
		totalTeams += mini;
	} else if (twoPro != 0 && zeroPro == 0) {
		int mini = min(oneProEng, oneProLan);
		totalTeams += mini;
		oneProLan -= mini;
		oneProEng -= mini;
		if (oneProEng == 0) {
			int mini = min(oneProLan, twoPro);
			twoPro -= mini;
			totalTeams += mini;
		} else if (oneProLan == 0) {
			int mini = min(oneProEng, twoPro);
			twoPro -= mini;
			totalTeams += mini;
		}
		if (twoPro != 0) {
