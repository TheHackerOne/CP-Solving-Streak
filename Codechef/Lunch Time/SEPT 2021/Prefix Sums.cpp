...
void solve() {
	...
	if (n % 4 == 0) {
		...
		for (int i = 1; i < a.size(); i++) {
			int next;
			if (first) next = a[i - 1] + 3, first = false, second = true;
			else if (second) next = a[i - 1] + 1, second = false, first = true;
			a[i] = next;
		}
		first = false, second = true;
		for (int i = 1; i < a.size(); i++) {
			int next;
			if (first) next = b[i - 1] + 3, first = false, second = true;
			else if (second) next = b[i - 1] + 1, second = false, first = true;
			b[i] = next;
		}
		for (auto i : a) cout << i << " ";
		cout << nline;
		for (int i = 1; i < half - 2; i += 2) {
			swap(b[i], b[i + 1]);
		}
		for (auto i : b) cout << i << " ";
		cout << nline;
