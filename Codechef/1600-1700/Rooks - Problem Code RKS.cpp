...
void solve() {
    ...
    int r, c;
    unordered_set<int> rowUsed, colUsed;
    vector<int> freeRow, freeCol;
    for(int i=0;i<k;i++){
        cin>>r>>c;
        rowUsed.insert(r-1);
        colUsed.insert(c-1);
    ...
    for(int i=n;i>=0;i--){
        if(rowUsed.find(i) == rowUsed.end()) freeRow.push_back(i);
        if(colUsed.find(i) == colUsed.end()) freeCol.push_back(i);
    }

    for(int c=0;c<n;c++){
        if(colUsed.find(c) == colUsed.end()){
            int freeR = freeRow.back();
            freeRow.pop_back();
            ans.push_back({freeR, c});
        }
    }
