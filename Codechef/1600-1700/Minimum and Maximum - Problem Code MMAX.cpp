...
void solve() {
    ...
    for(int i=0;i<kstr.length();i++){
        res = (res*10 + (kstr[i]-'0'))%n;
    }

    ll k = res;

    ll minChocolateInEachCell = k/n;
    ll rem = k%n;
    debug(k)
    debug(n)
    debug(k%n)

    ll maxChocCells = rem; // 2
    ll minChocCells = n-maxChocCells; // 1
    debug(maxChocCells)
    debug(minChocCells)
    ...
    if(maxChocCells == minChocCells){
        cout<<2*maxChocCells-1<<nline;
    }else if(maxChocCells > minChocCells){
        cout<<2*minChocCells<<nline;
    }else{
        cout<<2*maxChocCells<<nline;
    }
}
