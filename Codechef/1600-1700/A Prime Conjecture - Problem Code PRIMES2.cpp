...
void solve() {
    while(1) {
        ...
        int sqpos = upper_bound(all(si), sq) - si.begin();
        int cbpos = upper_bound(all(si), cb) - si.begin();
        bool flag = 0;

        for (int i = 0; i <= sqpos; i++) {
            for (int j = 0; j <= cbpos; j++) {
                int p2 = si[i], p3 = si[j];
                int rem = n - (p2 * p2 + p3 * p3 * p3);
                if (rem <= 1) continue;
                int idx = lower_bound(all(si), rem) - si.begin();
                if (si[idx] == rem) {
                    cout << rem << " " << p2 << " " << p3 << nline;
                    flag = 1;
                    break;
                }
            }
