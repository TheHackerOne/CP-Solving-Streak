...
void solve() {
    int n, k, s;
    cin>>n>>k>>s; // cho per day, eat per day, survive for s days
    ...
    int remCho = cho;
    int daysDontNeedCho = remCho/n;
    int daysCanBuyCho = (s/7)*6 + (s%7);
    debug(daysCanBuyCho)
    debug(daysDontNeedCho)
    cout<<daysCanBuyCho - daysDontNeedCho<<nline;
}
