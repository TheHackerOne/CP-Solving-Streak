...
int recur(int idx, string &str, int n, unordered_map<int, int> &dp, vector<vector<bool>> &palCache){
    // base case
    if(idx == n) return 1;
    ...
    for(int i=idx;i<n;i++){
        if(palCache[idx][i]){
            res = mod_add(res, recur(i+1, str, n, dp, palCache),MOD);
        }
    }
    return dp[idx] = res;
}
...
void solve() {
    ...
    cin>>str;
    int n = str.length();
    vector<vector<bool>> palCache(n, vector<bool> (n, false));
    ...
    for(int g=0;g<n;g++){
        for(int r=0,c=g; c<n; r++,c++){
            if(g == 0){
                ...
                if(str[r] == str[c]) palCache[r][c] = true;
                else palCache[r][c] = false;
            }else{
                if(str[r] == str[c] and palCache[r+1][c-1]){
                    palCache[r][c] = true;
                }else{
                    palCache[r][c] = false;
                }
    ...
    cout<<recur(0, str, str.length(), dp, palCache)<<nline;
}
