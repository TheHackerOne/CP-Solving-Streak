...
void solve() {
    ...
    for(ll i=0;i<n;i++){
        ll currComp = arr[i].second;
        lis[i] = currComp;
        ll st = arr[i].first.first, en = arr[i].first.second;
        for(ll j=0;j<i;j++){
            ll s = arr[j].first.first, e = arr[j].first.second;
            if(e <= st)
                lis[i] = max(lis[i], currComp + lis[j]);
        }
    }
    debug(arr)
    debug(lis)

    cout<<*max_element(all(lis))<<nline;
}
