...
void fillDPArray(){
    dp.resize(pals.size()+1, vector<int> (4*1e4+1, 1));

    for(int i=1;i<=pals.size();i++){
        for(int j=1;j<=4*1e4;j++){
            int idx = i-1;
            // cant take the current element
            if(i == 1){
                dp[i][j] = 1;
            }else if(pals[idx] > j){
                dp[i][j] = dp[i-1][j];
            }else{
                dp[i][j] = mod_add(dp[i-1][j], dp[i][j-pals[idx]], MOD);
            }
        }
    }
