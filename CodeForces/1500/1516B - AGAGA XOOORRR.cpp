...
void solve() {
    ...
    vector<ll> arr(n), preXor(n);

    for(ll i=0;i<n;i++){
        cin>>arr[i];
        if(i == 0) preXor[i] = arr[i];
        else preXor[i] = preXor[i-1]^arr[i];
    }
    ...
    for(ll i=1;i<n;i++){
        ll first = preXor[i-1], second = preXor[n-1] ^ preXor[i-1];
        debug(first)
        debug(second)
        if(first == second){
            cout<<"YES"<<nline;
    ...
    for(ll i=1;i<n;i++){
        for(ll j=i+1;j<n;j++){
            ll first = preXor[i-1], second = preXor[j-1] ^ preXor[i-1], third = preXor[n-1] ^ preXor[j-1];
