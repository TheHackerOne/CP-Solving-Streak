...
void solve() {
    ...
    string str;
    cin>>str;
    char belowChar = 'a',prevBelow = 'a', bottomUp = 'a';
    for(int i=0;i<n;i++){
        if(k == 0){
            if(str[i] <= prevBelow) str[i] = 'a';
            else if(str[i] >= bottomUp and str[i] <= belowChar) str[i] = bottomUp;
            continue;
        }
        if(str[i] <= prevBelow) str[i] = 'a';
        else{
            debug(prevBelow)
            int diff = (int)str[i] - (int)prevBelow;
            if(k >= diff){
                debug("h1")
                k -= diff;
                prevBelow = str[i];
