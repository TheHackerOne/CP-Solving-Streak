...
void solve() {
    ...
    if(k == n-1){
        cout<<n-2<<" "<<n-1<<nline;
        cout<<0<<" "<<1<<nline;
        cout<<n/2-1<<" "<<n-(n/2)+1<<nline;
        cout<<n/2-2<<" "<<n/2<<nline;
        unordered_map<int, int> mp;
        mp[0] = 1, mp[1] = 1, mp[n-1] = 1, mp[n-2] = 1, mp[n/2-1] = 1, mp[n/2-2]= 1;
        for(int i=0;i<n/2;i++){
            if(mp.find(i) == mp.end())
                cout<<i<<" "<<n-i-1<<nline;
        }
    ...
    int kpartner = n - k - 1;
    int rempartner = 0;
    cout<<rempartner<<" "<<kpartner<<nline;
    cout<<k<<" "<<n-1<<nline;

    for(int i=0;i<n/2;i++){
        if(i == k or i == kpartner or i == rempartner) continue;
        cout<<i<<" "<<n-i-1<<nline;
    }
}
