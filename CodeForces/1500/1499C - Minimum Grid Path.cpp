...
void solve() {
    ...
    ll minRes = n*arr[0] + n*arr[1];
    ll oddSum = arr[1], evenSum = arr[0];
    ll oddMin = arr[1], evenMin = arr[0];
    ll oddCnt = 1, evenCnt = 1;

    for(ll i=2;i<n;i++){  
        if(i%2 == 0) evenMin = min(evenMin, arr[i]), evenSum += arr[i], evenCnt++;
        else oddMin = min(oddMin, arr[i]), oddSum += arr[i], oddCnt++;

        ll res = oddSum + (n-oddCnt)*oddMin + evenSum + (n-evenCnt)*evenMin;
