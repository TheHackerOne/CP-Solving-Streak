...
void solve() {
    ...
    cin>>n;
    vector<int> arr(n, 0);
    vector<int> minPer(n, 0), maxPer(n, 0);
    ...
    for(int i=0;i<intervals.size()-1;i++){
        int st = intervals[i], en = intervals[i+1];
        int num = arr[st];
        // minprev
        for(int j=st+1;j<en;j++){
            debug(j)
            debug(smallestNum)
            while(mp1[smallestNum] == true){
                smallestNum++;
            }
            minPer[j] = smallestNum;
            mp1[smallestNum++] = 1;
        }
    }

    if(intervals[intervals.size()-1] != n-1){
        int st = intervals[intervals.size()-1], en = n;
        for(int j=st+1;j<en;j++){
            while(mp1[smallestNum] == true){
                smallestNum++;
            }
    ...
    for(auto i:minPer)cout<<i<<" ";
        cout<<nline;
    for(auto i:maxPer) cout<<i<<" ";
        cout<<nline;
}
