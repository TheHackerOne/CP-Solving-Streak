...
void solve() {
    ...
    int sr = 1+k, sc = 1+k, er = n, ec = m-k;


    for(int i=sr;i<=er;i++){
        for(int j=sc;j<=ec;j++){
            if(mat[i][j] == '*' and (mat[i-1][j-1] == '*' or mat[i-1][j-1] == '+') and (mat[i-1][j+1] == '*' or mat[i-1][j+1] == '+')){
                int r = i-1, c1 = j-1, c2 = j+1;
                mat[i][j] = '+';
                while(r >= 1 and (c1 >= 1 and (mat[r][c1] == '*' or mat[r][c1] == '+')) and (c2 <= m and (mat[r][c2] == '*' or mat[r][c2] == '+'))){
                    mat[r][c1] = '+';
                    mat[r][c2] = '+';
                    c1--, c2++, r--;
                }
