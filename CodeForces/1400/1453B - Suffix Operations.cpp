...
void solve() {
    ...
    // if(arr[0] >= arr[1] and arr[1] >= arr[2] and abs(arr[0]-arr[1]) > maxDiff){
    ...
    // if(arr[n-3] >= arr[n-2] and arr[n-2] >= arr[n-1] and abs(arr[n-1]-arr[n-2]) > maxDiff){
    ...
    for(int i=1;i<n-1;i++){
        ...
        if(first >= second and third >= second){
            int minDiff = min(abs(first-second), abs(second-third));
            minDiff *= 2;
            if(minDiff > maxDiff){
                maxDiff = minDiff;
                idx = i;
            }
        }else if(first < second and third < second){
            int minDiff = min(abs(first-second), abs(second-third));
            minDiff *= 2;
            if(minDiff > maxDiff){
                maxDiff = minDiff;
