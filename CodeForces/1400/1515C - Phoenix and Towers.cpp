...
void solve() {
    ...
    sort(all(arr), [](pair<int, int> &a, pair<int, int> &b){ return a.first > b.first; });

    debug(arr)

    int steps = m, tower = 0;
    while(steps--){
        pq.push({arr[tower].first, tower+1});
        ans[arr[tower].second] = tower+1;
        tower++;
    }

    for(int i=m;i<n;i++){
        int block = arr[i].first;
        int realIdx = arr[i].second;

        // top min tower
        pair<int, int> p = pq.top();
        pq.pop();
        int towerNo = p.second;
        int towerHeight = p.first;
        ans[realIdx] = towerNo;
        towerHeight += block;
        pair<int, int> res = {towerHeight, towerNo};
        pq.push(res);
    }
