...
void solve() {
    ...
    int oddCount = 0;
    for(auto i:mp){
        totalEven += i.second&1?i.second-1:i.second;
        if(i.second&1){
            oddCount++;
        }
    }
    ...
    int counter = 2;
    while(counter*k <= totalEven){
        counter += 2;
    }
    counter-=2;
    debug(counter)
    int minLen = counter; // k strings with len minLen
    totalEven -= counter*k;
    int countOfGreaterPal = 0;
    if(oddCount == 0 and totalEven >= k){
        minLen++;
        totalEven = 0;
    }

    if(totalEven+oddCount >= k) minLen++;
    cout<<minLen<<nline;
}
