...
void solve() {
    ...
    for(ll i=0;i<n;i++){
        pre[i] = (i-1 >= 0 ? pre[i-1] : 0) + arr[i].first;
    }
    ...
    while(lo <= hi){
        ll mid = (lo+hi)/2;
        ll prePoints = (mid-1 >= 0 ? pre[mid-1] : 0);
        ll totalScore = prePoints + arr[mid].first;
        bool canWin = true;
        for(ll i=mid+1;i<n;i++){
            if(totalScore >= arr[i].first){
    ...
    for(int i=ansIdx;i<n;i++){
        players.push_back(arr[i].second+1);
    }
