...
void solve() {
    ...
    vector<ll> higherNeigh(n+1, 0);
    for(ll i=0;i<e;i++){
        ll u, v;
        cin>>u>>v;
        mp[u].insert(v);
        mp[v].insert(u);
        if(v > u) higherNeigh[u]++;
        if(u > v) higherNeigh[v]++;
    }
    ll cnt = 0;
    for(ll i=1;i<=n;i++){
        if(higherNeigh[i] == 0) cnt++;
    }
    debug(higherNeigh)
    debug(cnt)
    ...
    while(q--){
        ...
        if(type == 1){
            ...
            if(v > u){
                higherNeigh[u]++;
                if(higherNeigh[u] == 1) cnt--;
            } 
            if(u > v) {
                higherNeigh[v]++;
                if(higherNeigh[v] == 1) cnt--;
            }
            debug(higherNeigh)
            debug(cnt)
        }else if(type == 2){
            ...
            if(u > v){
                higherNeigh[v]--;
                if(higherNeigh[v] == 0) cnt++;
            }
            if(v > u) {
                higherNeigh[u]--;
                if(higherNeigh[u] == 0) cnt++;
            }
            mp[u].erase(v);
            mp[v].erase(u);
            debug(higherNeigh)
