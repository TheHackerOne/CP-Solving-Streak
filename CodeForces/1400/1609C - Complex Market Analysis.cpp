...
void solve() {
    ...
    for(ll i=0;i<primeIdx.size();i++){
        ll idx = primeIdx[i];

        // going left
        ll count1 = 0;
        ll prevIdx = idx-e;
        while(prevIdx >= 0 and arr[prevIdx] == 1){
            count1++;
            prevIdx -= e;
        }
        ...
        ll count2 = 0;
        ll nextIdx = idx+e;
        while(nextIdx < n and arr[nextIdx] == 1){
            count2++;
            nextIdx += e;
        }
        
        if(count1 and count2) ans += (count1+1)*(count2+1)-1;
        else if(count1 != 0) ans += count1;
        else if(count2 != 0) ans += count2;
    }
