...
void solve() {
	...
	for (int i = 0; i < n; i++) {
		if (s[i] == '-' || s[i] == initDirection) continue;
		else {
			hasCycle = false;
			break;
		}
	...
	for (int i = 0; i < n; i++) {
		char leftType = (i == 0 ? s[n - 1] : s[i - 1]);
		char rightType = s[i];
		if (leftType == '<' && rightType == '>') nonReturnable++;
		else if (leftType == '>' && rightType == '<') nonReturnable++;
		else if ((leftType == '<' && rightType == '<') || ( leftType == '>' && rightType == '>')) {
			if (!hasCycle) nonReturnable++;
		}
	}
	cout << n - nonReturnable << nline;
}
