...
void solve() {
	...
	debug(mp)
	int ans = INT_MIN;
	for (int i = 2; i <= 2 * maxi; i++) {
		int localAns = 0;
		for (int j = 1; j <= i / 2; j++) {
			if (i - j == j) localAns += (mp.find(j) == mp.end() ? 0 : mp[j] / 2);
			else {
				int first = j, second = i - j;
				localAns += min((mp.find(first) == mp.end() ? 0 : mp[first]), (mp.find(second) == mp.end() ? 0 : mp[second]));
			}
		}
