...
void solve() {
	int n;
	cin >> n;
	vector<pair<ll, ll>> miner, diamond;
	for (int i = 0; i < 2 * n; i++) {
		ll x, y;
		cin >> x >> y;
		if (x < 0) x = -x;
	...
	double totaldist = 0;
	for (int i = 0; i < n; i++) {
		pair<ll, ll> fir = miner[i];
		pair<double, double> sec = diamond[i];
		ll x = fir.second * fir.second + sec.first * sec.first;
		double dist = (double)sqrt(x);
		totaldist += dist;
	}
	precision(15);
