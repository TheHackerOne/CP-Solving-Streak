...
void solve() {
	...
	if (!neg) {
		...
		if (zero) cout << 0 << nline;
		else {
			ll ans = 1, i = n - 1, count = 0;
			while (count != 5) {
				ans *= a[i];
				i--;
				count++;
		...
		ll op1 = LLONG_MIN, op2 = LLONG_MIN, op3 = LLONG_MIN;
		op1 = a[n - 1] * a[n - 2] * a[n - 3] * a[n - 4] * a[n - 5];
		if (negc >= 4) {
			op2 = a[0] * a[1] * a[2] * a[3] * a[n - 1];
		}
		if (negc >= 2) {
			op3 = a[0] * a[1] * a[n - 3] * a[n - 2] * a[n - 1];
		}
		debug(op1)
		debug(op2)
		debug(op3)
		ll ans = max({op1, op2, op3});
		cout << ans << nline;
	}
}
