...
ll calc1(int a, int b, int x, int y, int n) {
	int diff1 = a - x;
	int diff2 = b - y;
	if (n >= diff1) {
		n -= diff1;
...
ll calc2(int a, int b, int x, int y, int n) {
	int diff1 = a - x;
	int diff2 = b - y;
	if (n >= diff2) {
		n -= diff2;
...
ll calc3(int a, int b, int x, int y, int n) {
	if (n % 2) {
		int n1 = n / 2, n2 = n / 2;
		n2++;
		if (a - x <= n1) {
...
void solve() {
	ll a, b, x, y, n;
	cin >> a >> b >> x >> y >> n;
	ll opt1 = calc1(a, b, x, y, n);
	ll opt2 = calc2(a, b, x, y, n);
	ll opt3 = calc3(a, b, x, y, n);
	debug(opt1)
	debug(opt2)
	debug(opt3)
	cout << min({opt1, opt2, opt3}) << nline;
}
