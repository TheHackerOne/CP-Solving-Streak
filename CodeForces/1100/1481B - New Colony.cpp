...
int firstDipIndex(vector<int> &a) {
	int i = 1;
	while ((a[i] >= a[i + 1]) && ((i + 1) <= (a.size() - 1))) {
		i++;
	}
...
bool equalHeights(vector<int> &a) {
	int he = a[1];
...
void solve() {
	...
	int lastBould;
	while (!equalHeights(a) && boulders > 0) {
		int idx = firstDipIndex(a);
		if (idx == -1) {
			cout << -1 << nline;
			debug("-----")
			return;
