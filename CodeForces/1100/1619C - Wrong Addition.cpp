...
void solve() {
    ...
    cin>>a>>s;
    string ans = "";
    int alast = a.length()-1, slast = s.length()-1;
    while(alast >= 0 && slast >= 0){
        // cout<<alast<<" "<<slast<<nline;
        int anum = a[alast--]-'0', snum = s[slast--]-'0';
        int diff = snum-anum;
        // cout<<"diff -> "<<diff<<nline;
        if(diff >= 0){
            ans = to_string(diff)+ans;
        }else{
            if(slast >= 0 and s[slast] == '1')
                snum += (s[slast--]-'0')*10;
            else{
                cout<<-1<<nline;
                return;
