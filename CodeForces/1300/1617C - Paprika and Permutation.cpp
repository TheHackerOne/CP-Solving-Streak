...
void solve() {
    ...
    // cout<<(*mp.begin()).first<<nline;
    // for(auto i:mp) cout<<i.first<<" "<<i.second<<nline;
    sort(all(arr));
    debug(arr)
    int minOperation = 0;
    for(int i=n-1;i>=0;i--){
        int largestAvaNum = (*mp.begin()).first;
        int partition = (*mp.begin()).first/2+1;
        if(mp.find(i+1) != mp.end() and mp[i+1] >= 1){
            mp[i+1]--;
            if(mp[i+1] == 0) mp.erase(i+1);
        }else if((i+1) <= (largestAvaNum)-partition){
            minOperation++;
            mp[largestAvaNum]--;
            if(mp[largestAvaNum] == 0) mp.erase(largestAvaNum);
        }else{
            cout<<-1<<nline;
            return;
        }
    }
    cout<<minOperation<<nline;
}
