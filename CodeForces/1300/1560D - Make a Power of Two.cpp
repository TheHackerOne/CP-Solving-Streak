...
void solve() {
    ...
    while(init <= 1e18){
        ...
        ll num = n, powerOf2 = init;
        ll numAfterRemoval = 0, moves = 0;

        string no = to_string(n), po2 = to_string(init);
        
        int f = 0, s = 0, removeCount = 0;
        while(f < no.length() and s < po2.length()){
            if(no[f] == po2[s]){
                f++, s++;
                continue;
            }
        ...
        int toAdd = po2.length()-s;
        if(f < no.length()){
            removeCount += no.length()-f;
        }

        moves = removeCount + toAdd;
        // if(init == 1024) cout<<moves<<endl;
        init *= 2;
