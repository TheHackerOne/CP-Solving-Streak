...
pair<int, int> dfs(int src, unordered_map<int, vector<int>> &mp, string&str, int &cnt){
    int black = 0, white = 0;
    if(str[src-1] == 'B') black++;
    else white++;

    for(auto neigh:mp[src]){
        pair<int, int> res = dfs(neigh, mp, str, cnt);
        // black, white
...
void solve() {
    ...
    for(int i=0;i<n-1;i++){
        mp[arr[i]].push_back(i+2);
    }

    int cnt = 0;
    debug(tar)

    pair<int, int> res = dfs(1, mp, tar, cnt);
