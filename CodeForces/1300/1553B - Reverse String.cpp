...
void findTar(string str, string tar, int idx, unordered_map<string, int> &allPoss){

    int tarLen = tar.length();
    
    for(int right=0;right<tarLen;right++){
        string res = str.substr(idx, right+1);
        int remLen = tarLen - (right+1);
        for(int left=0;left<remLen;left++){
            int sidx = idx+right-left-1;
            res += str[sidx];
        }
        allPoss[res] = 1;
    }
...
void solve() {
    ...
    vector<int> idx = mp[tar[0]];

    unordered_map<string, int> allPoss;
