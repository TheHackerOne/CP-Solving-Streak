...
int def(int idx, string &str){
    // base case
    if(dp[idx] != -1) return dp[idx];
    if(idx >= str.length()) return 0;
    if(idx == str.length()-1) return 1;
    if(str[idx] == str[idx+1]) return def(idx+2, str);

    // memo

    // main logic
    int j;
    for(j=idx+1;j<str.length();j++){
        if(str[idx] == str[j]) break;
    }
    int res1 = 1 + def(idx+1, str);
    if(j == str.length()) return res1;
    int res2 = (j == str.length() ? INT_MAX : (j-idx-1 + def(j+1, str)));
    return dp[idx] = min(res1, res2);
}
