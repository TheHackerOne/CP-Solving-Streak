...
void solve() {
	...
	ll k;
	cin >> k;
	vector<ll> defence(k), attack(k);
	for (ll i = 0; i < k; i++) {
		cin >> defence[i] >> attack[i];
	}
	for (ll i = 0; i < k; i++) {
		ll lTotal = total;
		ll coins1 = 0, coins2 = 0;
		ll ddragon = defence[i], dattack = attack[i];
		debug(ddragon)
		debug(dattack)
		auto itr = a1.lower_bound(ddragon);
		if (itr == a1.end()) {
			debug("here")
			coins1 = LLONG_MAX;
		}
		...
		if (*itr != a[0]) {
			...
			llTotal -= *itr1;
			if (llTotal < dattack)
				coins2 += dattack - llTotal;
		}
		debug(coins1)
		debug(coins2)
		debug("---")
		if (!sposs) cout << coins1 << nline;
		else cout << min(coins2, coins1) << endl;
	}
}
