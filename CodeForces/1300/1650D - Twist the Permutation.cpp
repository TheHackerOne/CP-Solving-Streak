...
void doRightShift(vector<int> &arr, int rightShift, int n){
    reverse(arr, 0, n-rightShift-1);
    reverse(arr, n-rightShift, n-1);
    reverse(arr, 0, n-1);
}
...
void solve() {
    ...
    for(int i=n-1;i>=1;i--){
        ...
        int rightShift = (idx + 1)%(i+1);
        doRightShift(arr, (i+1)-rightShift, i+1);
        debug(arr)
        res[i] = rightShift;
    }
