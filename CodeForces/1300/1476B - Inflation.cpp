...
void solve() {
  ...
  vector<ll> pre(n, arr[0]);
  for(ll i=1;i<n;i++){
    pre[i] = pre[i-1] + arr[i];
  }

  ll maxIncrease = INT_MIN;
  ll idx = -1;

  for(ll i=1;i<n;i++){
    ll pi = arr[i]*100;
    ll den = pre[i-1];
    double ratio = pi/(double)den;
    if(ratio > k){
      if(idx == -1) idx = i;
      ll fix = ceil((pi-k*den)/(float)k);
      maxIncrease = max(maxIncrease, fix);
    }
  }

  if(maxIncrease == INT_MIN){
    cout<<0<<endl;
    return;
  }
