...
void solve() {
    ...
    vector<ll> ans(n);
    ll minMoves = 0;
    ll minMoves1 = 0, minMoves2 = 0;
    if(evenCount > oddCount){
        ll evenIdx = 0, oddIdx = 1;
        for(ll i=0;i<n;i++){
            ll num = arr[i];
            if(num&1){
                ans[oddIdx] = arr[i];
                minMoves += abs(oddIdx-i);
                oddIdx+=2;
            }else{
                ans[evenIdx] = arr[i];
                minMoves += abs(evenIdx-i);
                evenIdx += 2;
            }
        }
        debug(ans)
        debug(minMoves)
        cout<<minMoves/2<<nline;
        return;
    }else if(oddCount > evenCount){
        ll evenIdx = 1, oddIdx = 0;
        for(ll i=0;i<n;i++){
            ll num = arr[i];
            if(num&1){
                ans[oddIdx] = arr[i];
        ...
        for(ll i=0;i<n;i++){
            ...
            if(num&1){
                ans1[oddIdx] = arr[i];
                minMoves1 += abs(oddIdx-i);
                oddIdx+=2;
            }else{
                ans1[evenIdx] = arr[i];
                minMoves1 += abs(evenIdx-i);
                evenIdx += 2;
            }
        }
        evenIdx = 0, oddIdx = 1;
        for(ll i=0;i<n;i++){
            ...
            if(num&1){
                ans2[oddIdx] = arr[i];
                minMoves2 += abs(oddIdx-i);
                oddIdx+=2;
            }else{
                ans2[evenIdx] = arr[i];
                minMoves2 += abs(evenIdx-i);
                evenIdx += 2;
            }
        }
        if(minMoves2 > minMoves1) {
            debug(ans1)
            debug(minMoves1)
        }else{
            debug(ans2)
        ...
        cout<<min(minMoves1, minMoves2)/2<<nline;
        return;
    }
    cout<<-1<<nline;
    return;
