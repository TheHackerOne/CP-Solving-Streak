...
void solve() {
    ...
    // along the height
    double ho1 = abs(y2-y1), wo1 = abs(x2-x1);
    double dist1 = INT_MAX;
    if(ho1 + h1 <= h){
        bool up = false, down = false;
        if(h-y2 > y1) down = true;
        else up = true;

        if(up){
            dist1 = h1-y1>=0?h1-y1:0;
        }else{
            dist1 = y2-(h-h1)>=0?y2-(h-h1):0;
        }
    }
    double dist2 = INT_MAX;
    if(wo1 + w1 <= w){
        bool left = false, right = false;
        if(x1 > w-x2) right = true;
        else left = true;

        if(left){
            dist2 = x2-(w-w1)>=0?x2-(w-w1):0;
        }else{
            dist2 = w1-x1>=0?w1-x1:0;
        }
    }

    if(dist1 != INT_MAX or dist2 != INT_MAX){
        precision(12);
        cout<<min(dist1, dist2)<<nline;
        return;
    }
